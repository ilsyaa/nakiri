...
Command({
  ...
  run: async ({ m }) => {
    ...
    if (!lang) return m.reply(__('group.setlang.ex', { command: m.content.command, langs: langs.join(', ') }));
    if (!langs.includes(lang)) return m.reply(__('group.setlang.ex', { command: m.content.command, langs: langs.join(', ') }));

    await prisma.Group.update({ where: { id: m.db.group.id }, data: { lang }});
    m.reply(__('group.setlang.success', { lang }));
  }
});